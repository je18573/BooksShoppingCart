...
namespace BooksShoppingCart
{
    ...
    // 設定此應用程式中使用的應用程式角色管理員。RoleManager 在 ASP.NET Identity 中定義且由應用程式中使用。
    ...
    //@todo: 於網站建立完成後可移除
    //於第一次建立網站時((第一次建立identity相關資料表))，自動建立一組Admin帳號
    public class ApplicationDbInitializer : DropCreateDatabaseIfModelChanges<ApplicationDbContext>
    {
        ...
        public static void InitializeIdentityForEF(ApplicationDbContext db)
        {
            ...
            const string name = "admin@test.com";    //使用者帳號
            const string password = "Admin@1qaz";    //使用者密碼
            const string nickName = "AdminManager";  //使用者暱稱
            const string roleName = "Admin";         //使用者所屬群組
            ...
            if (user == null)
            {
                user = new ApplicationUser { UserName = name, Email = name, NickName = nickName };
                var result = userManager.Create(user, password);
                result = userManager.SetLockoutEnabled(user.Id, false);
            }

            //將admin@test.com帳號加入Admin群組
